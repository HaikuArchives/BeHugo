...
public:
	...
	MediaView(BRect frame, const char *name, uint32	resizeMask, uint32 flags = B_WILL_DRAW | B_FRAME_EVENTS);
	virtual ~MediaView();

//	status_t SetMediaSource(const char *path);
	status_t SetMediaSource(BDataIO *source);
	...
	virtual status_t SetVolume(float vol);
	virtual float Volume();

private:
	void InitObject();

	status_t SetVideoTrack(BMediaTrack *track, media_format *format);
	status_t SetAudioTrack(BMediaTrack *track, media_format *format);
